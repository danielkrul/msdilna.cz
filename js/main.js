...
function loadImages(folder)
{
	...
	var title = categories[folder].title;
	$('.galleryHeader .galCatTitle').text(title);
	$('.galleryHeader .galCatText').text(categories[folder].text);

	console.log(categories[folder].text);

	var imgCnt = categories[folder].imgCnt;

	for(var i = 0; i < imgCnt; i++)
	{
		var name = folder + (i + 1) + '.jpg';

		var s = '<li><div class="in-image">';
		s+= '<a href="images/' + folder + '/large/' + name + '" data-title="'+ title +'" data-lightbox="image-1">';
		s+= '<img src="images/' + folder + '/small/' + name + '" />';
		s+= '</a></div></li>';
